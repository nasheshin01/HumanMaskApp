...
class ASPPBlock(keras.Model):
    ...
    def call(self, x):
        stdConvOut = self.stdConv(x)
        smallDilationConvOut = self.smallDilationConv(x)
        mediumDilationConvOut = self.mediumDilationConv(x)
        largelDilationConvOut = self.largeDilationConv(x)
        convConcat = tf.concat([stdConvOut, smallDilationConvOut, mediumDilationConvOut, largelDilationConvOut], axis=3)
        mixConvOut = self.mixConv(convConcat)
